...
def update_company(
    ...
    existing_company = db.query(Company).filter(Company.company_name == company_name, Company.id != company_id).first()
    if existing_company:
...
def delete_company(company_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=422, detail='单位删除失败，请先处理关联数据') from exc
    return RedirectResponse(url='/companies/', status_code=303)
