...
def list_records(request: Request, db: Session = Depends(get_db), company_id: int | None = None, status: str = ''):
    ...
    return templates.TemplateResponse('fee_records/list.html', {'request': request, 'records': records, 'companies': companies, 'selected_company_id': company_id, 'selected_status': status, 'title': '收费记录'})
...
def new_record(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse('fee_records/form.html', {'request': request, 'companies': companies, 'pipeline_entries': pipeline_entries, 'statuses': statuses, 'title': '新增收费记录'})
